...
| 需求 | 請讀 |
| --- | --- |
| **總流程**：天條、三階段契約、交接物、`status.json` schema | [`rules/AGENTS.md`](rules/AGENTS.md) |
| **Phase 1 建模**：四階段降維、Model.md 契約、線性化 pattern、multi-agent | [`rules/Ph1_Modeling/model-design-guide.md`](rules/Ph1_Modeling/model-design-guide.md) |
...
| 資料夾 | 內容 | 維護原則 |
| --- | --- | --- |
| `rules/` | **唯一的規則來源**：總流程 1 檔 + 三個 phase 資料夾 | 同一條硬規則只在權威文件定義，其餘文件以連結引用；NEVER 新增第二份談同一階段的規則檔 |
...
3. `rules/` 底下的每份 `.md` 都會進入 agent 的 context，篇幅即成本——寫進去之前先問「這條規則有沒有已經在別處講過」。
