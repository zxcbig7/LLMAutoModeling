# Phase 2 AI 專案產出一致性契約（Model.md → Code）

> **內部 AI enforcement spec，不是交付給使用者閱讀或勾選的文件。**任何 agent 產生、補修或驗證 `Projects/<Project>/` 時，MUST 以本檔機械檢查輸出是否符合唯一專案架構。未通過 = 繼續修正，不得宣告 Phase 2 完成。
>
> 完整規範與 API 以同層 [`optimfoundation-api-guide.md`](optimfoundation-api-guide.md) 為準；本檔將其轉成統一專案輸出的可驗證 contract。它不解釋選擇、不提供人工作業流程，也不取代 Model.md。
...
- [ ] 若專案已進入 Phase 3，唯一合法延伸由 `../tuning/solver-tuning-guide.md` 定義：專案根的 `TuningHistory.md`、`status.json` 與 `Experiments/`。`Experiments/` 只可包含 `<Project>-tuning-r<N>.csv`、`.json`、`-trajectory.csv` 三件 archive；不得以此為由新增其他資料夾。
- [ ] NEVER 建立 `Common/`、`Helpers/`、`Utils/`、`Services/`、`Models/`、`Infrastructure/`、`Domain/`、`Data/import/` 或任何依專案而變的資料夾。
- [ ] 每個手寫型別一型別一檔，且只放入此型別責任所屬的固定資料夾；不得集中成 `Sets.cs`、`Variables.cs`、`Constraints.cs` 或通用 helper 檔。
...
## 9.1 Program.cs 四段不可變模板

- [ ] `Program.cs` 按順序各有且只有一個逐字標記：`// 1. import 段落`、`// 2. 模型段落`、`// 3. 實驗段落`、`// 4. 正式跑段落`；缺一、重複、改名或重排一律 FAIL。
