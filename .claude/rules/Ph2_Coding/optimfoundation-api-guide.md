...
| 可行解難找   | `nodeSelect = 0`（DFS）→ `intSolLimit` / `nodeLimit` → 放寬 `epGap` → `mipEmphasis = 1`（僅限模型本身有解、只是難找） |
...
engine.CreateLeSoft(rhs, penalty, name); // 只在 Model.md 明寫 soft 時
engine.CreateGeSoft(rhs, penalty, name); // 同上
engine.CreateEqSoft(rhs, penalty, name); // 同上
...
├── DesignBases.cs ModelElementBase / VariableBase / ParameterBase / ConstraintBase / ISetBrick / SetBase<T>
...
public abstract class SetBase<T> : ISetBrick, IReadOnlyList<T>
{
    ...
    public T this[int index] { get; } // IReadOnlyList<T>，可直接索引 / foreach / LINQ
...
public virtual bool CreateLeSoft(double rhs, double penalty, string name); // 只在 Model.md 明寫 soft 時使用
public virtual bool CreateGeSoft(double rhs, double penalty, string name); // 同上
public virtual bool CreateEqSoft(double rhs, double penalty, string name); // 同上
...
public sealed class OptProject : IDisposable
{
    ...
    public TimeSpan totalTimeSpan { get; } // Execute() 結束後填入
    public Stopwatch buildModelTimer { get; } // 建模階段耗時
    public Stopwatch totalTimer { get; } // 全程耗時
}
...
public sealed class Trial
{
    ...
    public ConfigSnapshot Config { get; set; } // tunable + SolverSpecific，可重現設定快照
...
public sealed class ConfigSnapshot
{
    ...
    public Dictionary<string, object> Tunable { get; set; } // ITunableConfig 的抽象旋鈕
    public Dictionary<string, object> SolverSpecific { get; set; } // CplexConfig 專屬欄位；promotion 寫回 baseline 讀這裡
}
