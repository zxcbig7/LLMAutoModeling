...
> **黃金順序（限建模當下）**：**還在 Phase 1、模型尚未定版**時，先做「模型優化」（結構、變數型態、邊界、對稱性），再做「參數優化」（solver 旋鈕）——模型改一刀的效益通常遠大於調十個參數。
> **模型已定版後（Phase 3）只剩旋鈕一條路**：模型與資料凍結、已 feasible，只調 `CplexConfig`，依 `$FW/MILP Model/CLAUDE.md` 天條。本文件 §1（模型層手段）與 §3（callback / warm start）**只適用建模當下**；Phase 3 讀本檔是為了 §2 的旋鈕與 §4 的實驗方法論。旋鈕連調 3 輪無改善 → 停止 tuning 並回報，要不要回頭改模型結構是使用者的決定，不是 tuning 的下一步。
...
| IV 整數 | 需分支定界 | 中 | `BuildIVs<VariableI_*>(...)` | 數量（台數、批次）等不可分割但範圍大 |
| BV 二元 | 需分支，且常引發對稱與弱鬆弛 | 最高 | `BuildBVs<VariableB_*>(...)` | 是/否決策、選址、開關 |
...
> 黃金順序（限建模當下）：Phase 1 模型未定版時先做模型優化（§1），再用 `OptExperiment` 掃 solver 旋鈕（§2）。模型已定版的 Phase 3 反過來——先旋鈕，見檔頭。
