...
> **通用天條唯一權威在 [`../../rules/AGENTS.md`](../../rules/AGENTS.md)**；本檔是 interactive 路線的三階段流程細則（phase gate / 建模轉譯專屬規則），各階段細節在同資料夾的 `phase-*.md`。所有路徑相對本 repo。
...
- API 對照：[`../../rules/Ph2_Coding/optimfoundation-api-guide.md`](../../rules/Ph2_Coding/optimfoundation-api-guide.md)（§4 Pool、§6 取解、§8 Experiment、§9 完整簽名與黑名單）
- 專案模板：[`../../../Template/`](../../../Template/)
- DLL 唯一來源：[`../../../dlls/`](../../../dlls/)（csproj HintPath 一律指這裡）
- 可運作範例：`../../../Projects/HospitalRostering_Generator`（generator + OptModel，**唯一起手參考**）；`../../../Projects/HospitalRostering_Manual` 是已廢止寫法的歷史對照，NEVER 照抄

> 框架 13 章 API 手冊 `developer-guide.md` 在**同層 sibling 資料夾** `OptimFoundation/`（本 repo 外）；`rules/Ph2_Coding/optimfoundation-api-guide.md` 為 repo 內唯一來源，developer-guide 為進階補充，非硬相依。

## Fatal

通用天條（數值保真、API 白名單、框架唯讀、相對路徑、DLL 引用）**全量見 [`../../rules/AGENTS.md`](../../rules/AGENTS.md)**——不在此重複。本檔上方的 phase gate / 命名 / 數學一致性為 interactive 路線專屬補充。interactive 額外硬規則：**模型未經使用者確認前 NEVER 產任何 `.cs`**（使用者明說「模型確認過直接寫」視同通過 gate）。
- NEVER 用絕對路徑
</content>
