...
{
    /// <summary>
    /// 參數掃描實驗（dotnet run -- experiment）。與 solve 模式共用 ModelComposer，
    ...
    public static class ExperimentRunner
    {
        ...
        {
            Logging.SetLogFileName("RosteringProblem_new_Experiment");

            var exp = new Experiment(
                "rostering-new-tuning",
            ...
            foreach (var (label, tune) in variants)
            {
                ...
                engine.Build();
                ModelComposer.BuildVariables(engine, data);
                ModelComposer.BuildModel(engine, data);
