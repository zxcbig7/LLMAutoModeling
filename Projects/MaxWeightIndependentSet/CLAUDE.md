...
2026-07-14 已從舊的 `stages/` + `csharp/` 兩層結構扁平化（`csharp/` 上提、`stages/` 數學模型精華併入 `Model/`）。

⚠ 但本專案**仍沿用早期 `Model/` + `Project/` 兩夾慣例，非框架標準六夾**（Set/Parameter/Variable/Constraint/Objective 各自獨立）。作為 tuning-study 範例保留原貌，新專案 NEVER 照抄本結構——標準結構見 `../CLAUDE.md`。

| 路徑 | 內容 |
|------|------|
| `Model/` | 數學模型（`MaxWeightIndependentSet_Model.md`）+ Parameters/Variables/Constraints/ObjectiveFunction |
| `Project/` | `Dataload` / `VariableCreate` / `BuildModel` / `Project` / `ExperimentRunner`（tuning sweep） |
